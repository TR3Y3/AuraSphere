...
- Current phase: **F6 (complete)** — Tracking: check-calls/pings + a simple map + ETA + status auto-advance. This was the last core F-phase. Next: signature/product-track polish or deferred cleanup (companies→shippers rename, remove legacy deals backend).
- Last completed: F6 — load tracking with check-calls, a route map, ETA, and a status auto-advance hook.
