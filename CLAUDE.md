...
- Current phase: **Product track — DAT market rates integration (complete)**. Next candidates: Gmail email-logging, factoring, ELD; or deferred cleanup (companies→shippers rename, remove legacy deals backend).
- Last completed: DAT market rates (spot rate lookup by lane + equipment) with a stub mode + a real-provider seam.
