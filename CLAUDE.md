...
**Phase 5 — launch plan (agreed direction):**
- Infra: stay on Render (blueprint already proven) but **upgrade off free tier before go-live** — free web services spin down after idle (≈50s cold start = "app is broken" to testers) and free Postgres has no backups and a limited lifetime. Backend → Starter, Postgres → paid tier w/ daily backups. Frontend static stays free. Custom domain: app.<domain> + api.<domain>, `COOKIE_DOMAIN=.<domain>` (lets SameSite go back to `lax`), update FRONTEND_ORIGIN/VITE_API_URL, verify the same domain in Resend.
- "Flawless" bar for the polish pass: global exception handler + React error boundary (Phase-2 S6), skeleton/loading states on every list+detail, purposeful empty states (partially done), friendly 404, favicon/title/meta, mobile pass on the app bar + board + detail pages (board horizontal-scrolls by design; verify tabs/KPI wrap), form validation messages on all money/date fields.
- Ship THIS WEEK (order): 1) fix B1 accept race + B3 board auth + S5 409-on-duplicate + S6 handlers/boundary; 2) infra: paid tiers, domain, Resend live (B2), FEEDBACK_EMAIL set; 3) polish pass above; 4) seed CorTrans org + real data, 2-device smoke test (desktop+phone); 5) invite first 2-3 testers, then widen. FAST-FOLLOW (next 2 wks): S1-S4 hardening, doc generation (#1) + cover→rate-con→email flow (#2), saved views/CSV export (#9), command-bar actions (#6), churn cues (#5). LATER: email-to-load AI, invoicing/QuickBooks, shipper portal, carrier app, EDI.
- Rollback: Render one-click rollback to any previous deploy (build images are kept); DB policy = **additive-only migrations during launch window** (no drops/renames; every migration must be roundtrip-tested — already verified base↔head), so old code always runs against new schema; daily Postgres backups + pre-deploy manual snapshot; stub-seam env vars act as feature kill-switches (email/billing/DAT/ELD/vetting can each be flipped back to stub/console independently); incident path = flip env → rollback deploy → restore snapshot (worst case), comms via the in-app feedback channel + direct email to the tester list.



- Current phase: **Prompt 5 — coverage rules + load feed (complete).**
  - **Cover requires carrier** (item 1): `CARRIER_REQUIRED_STATUSES` (covered→invoiced) guard in `loads._apply_status` + on direct create — 422 without a carrier; guard evaluates the *incoming* carrier on combined PATCHes. UI: booked-status forward buttons disabled with a why-tooltip.
  - **Auto-miles** (item 2): `lib/cityCoords.ts` — static lat/lng for the curated `US_CITIES` + haversine × 1.18 road factor; LoadForm auto-fills `total_miles` when both lane endpoints are known cities (never overwrites hand-typed miles; replaces only its own estimate). No API/key needed; exact routed miles can land later behind a `MILES_MODE`-style seam.
  - **Equipment suggestions** (item 3): `lib/equipment.tsx` datalist (Dry Van, Reefer, Flatbed, Step Deck, RGN/Lowboy, Specialized, Power Only, Expedite, Container/Intermodal, Box Truck) on LoadForm — dropdown-first, free text preserved.
  - **Uncover + reason** (item 4): `POST /api/loads/{id}/uncover` {reason ∈ TONU/Bounced/Carrier Fault/Rate Dispute/Capacity Fell Through/Other, note (required for Other)} — covered/dispatched/in_transit → tendered, clears carrier/rate/offer-lock, declines the accepted Quote Desk option, posts the reason to the feed. UI: ↩ Uncover button + reason modal on booked loads. (Decision on record: reason=TONU still reverts to tendered for re-covering; the separate terminal-TONU action remains for loads that die there.)
  - **Load feed** (item 5, plan approved then built): `activities` extended with `kind` ('user'|'system'), `event_type`, `meta` JSON, `mentions` JSON + `users.mentions_seen_at` (migration `55c4f7ce4243`, roundtrip-tested). `app/events.py` (`log_event`/`log_status_change`/`log_carrier_change`) emits from: load create, both status endpoints, carrier add/remove, uncover, Quote Desk accept ("Covered with X at $rate"), ratecon sent/signed, doc upload (broker + carrier portal), tracking advance. System rows are API-read-only (403 on patch/delete). Noise policy: field edits, DAT posts, GPS pings stay out of the feed. Timeline component = the thread: compact one-line system rows (icon per event_type), "Notes only" toggle, **@mention composer** (type @ → teammate dropdown → ids stored in `mentions`, validated in-org), 📎 attach on load feeds (uploads via the documents endpoint → the doc event lands in the feed), 10s poll on load feeds. **Mention bell** in the app bar (`GET /api/activities/mentions/unseen` + `POST .../seen`, 30s poll) — badge count, dropdown linking to the load, marks seen on open. In-app only; email notifications are a fast-follow once Resend is live.
  - Also: market-rate panel now shows a "demo data — set a DAT key" badge in stub mode (audit item 6a).
  - 152 tests pass (13 new in `test_prompt5.py`: guards incl. carrier+status-in-one-PATCH, uncover flow/validation/option-decline/isolation, feed emits, system-row read-only, mentions flow + cross-org reject). Pre-existing tests that booked loads without carriers were updated to attach one (that's the new rule working). Verified live (Playwright: Cover disabled without carrier → uncover modal → TENDERED, feed shows "Uncovered — Bounced (was RedClay Carriers)" + carrier/status/created events, auto-miles ATL→CLT=267, equipment datalist, @mention dropdown).
  - On-record findings (item 6): market-rate panel = `app/dat.py` stub math unless `DAT_MODE=dat`+key (every number so far is demo, deterministic per lane); "Mark Lost" = ungated status→lost from the action row or header dropdown, no reason/confirm (candidate: route through the uncover-reason machinery later); test-user paths exist = /signup (fresh org) + Settings→Team invite (set-password link only in server logs while email=console) + `python -m app.seed`.
- Earlier phase: **Carrier app v1 (complete)** — the deferred flagship, shipped as a token-linked mobile-web portal on the seams planted earlier:
  - Backend: `carriers.portal_token_hash` (hash-only, rotate=revoke; migration `22f165fcba64`). Broker endpoints `POST/DELETE /api/carriers/{id}/portal-link` (link shown once); `CarrierOut.portal_enabled` computed (hash excluded from output). Public portal router `app/routers/portal.py` (`?token=` auth → ONE carrier + its org; deactivated carrier → 403): `GET /meta` (org brand), `GET /loads/available` (org's tendered loads, **carrier-safe shape — no customer_rate/margin/shipper**), `POST /loads/{id}/offer` (→ `offers.create_option(source='carrier_app')` — lands live in the Quote Desk with the 'app' badge + MC auto-match), `GET /loads/mine` (their loads covered→invoiced, their `carrier_rate` only), `POST /loads/{id}/documents` (POD/BOL multipart, own loads only), `POST /loads/{id}/ping` (phone GPS → check_calls → tracking map). 141 tests pass (token auth, org+carrier scoping, no-rate-leak assertion, offer/upload/ping, revoke+deactivation).
  - Frontend: public `/portal` route (`src/pages/CarrierPortal.tsx`) — mobile-first, org-branded (accent color), Load board / My loads tabs, make-offer inline form, **Share location** (geolocation.watchPosition throttled ~1/min → pings), **Upload POD/BOL** (camera/file), 20s auto-refresh. Broker side: **Carrier portal panel** on the carrier Overview (generate/copy/rotate/revoke link).
  - Verified live (Playwright, phone viewport): branded portal → offer $1,850 → appears in broker Quote Desk as BlueRoad Express · MC556677 [App] with margin computed; My loads shows carrier pay + share-location + upload buttons.
  - v2 candidates: instant-book on green-lit posted rates, portal ratecon sign deep-link, push notifications, carrier-side load history.
- Earlier phase: **Product/UX audit implementation (PROMPT 4, complete)** — audit delivered in chat (9 sections); all self-greenlit items shipped:
...
- Earlier phase: **Feature Round 2 (complete)** — evaluate-then-build pass, all greenlit items shipped:
  ...
  - **Slice D (items 6+7)**: **Live options page + rate-con e-sign + Offered lock.** `load_options` + `mc_number`/`dot_number`/`source` ('manual'|'carrier_app'); **`app/offers.py` is the carrier-app seam** (`create_option()` shared service — the future carrier app calls it with source='carrier_app'; MC/DOT auto-matches org carriers; `carrier_light` derived live: green=vetted-clear+compliant, orange=review/unvetted, red=fail/deactivated, grey=not-in-system). `rate_confirmations` table (token-hash sign links, doc hash, signer name/IP/timestamp); `app/ratecon.py` renders print-ready HTML rate cons (no PDF dep). Endpoints: `POST .../options/{id}/cover` (green-only: atomic accept + rate con + emailed sign link), `POST /api/loads/{id}/offer` (atomic tendered→offered lock to one carrier, TTL 5–15 min via `OFFER_TTL_MINUTES`), public `GET/POST /api/sign/{token}` (single-use; signing an offered load covers it atomically + files the signed copy under Documents). **Lazy expiry** (no scheduler): `offers.resolve_offer_expiry` on load reads + bulk revert in list — offered loads revert to tendered when the window lapses; accept respects the offered-lock (409). Migration `8997a3f593b6` (batch mode for the loads FK). Frontend: QuoteDesk shows traffic-light dots + MC column + "✓ Cover Load" (green) / "⏳ Offer" buttons + offered-countdown banner; public `/sign` page (carrier-facing, no login; the carrier app deep-links here later). 135 tests pass; full flow verified live (Playwright: green/grey lights → Cover Load → sign page → signed → covered + doc filed).
  - Evaluation verdicts on record: shippers+contacts merge = NO-GO (breaks carrier-side contacts/load semantics); live map = GO-guarded (Leaflet, pending); item 3 skipped by recommendation.
